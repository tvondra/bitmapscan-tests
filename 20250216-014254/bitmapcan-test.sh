...
echo "device build rows dataset relpages workers wm eic matches distinct run caching optimal timing" > $OUTDIR/results.csv
...
for rows in 1000000 10000000 100000000; do
	...
	for dev in raid-sata raid-nvme data; do
					...
					for wm in 128 $((4*1024)) $((64*1024)); do
						...
						for eic in 0 1 8 16 32; do
							...
							for ioc in 16 1 32 8; do
								...
								for build in master patched-0001 patched-0002 patched-0003 patched-0004; do
...
EOF
										...
										echo $dev $build $rows $dataset $relpages $workers $wm $eic $ioc $matches $distinct $run uncached $optimal $t >> $OUTDIR/results.csv

										echo "========== dev: $dev  build: $build  rows: $rows  dataset: $dataset  relpages: $relpages  workers: $workers  work_mem: $wm  effective_io_concurrency: $eic  io_combine_limit: $ioc  matches: $matches  distinct: $distinct  run: $run  caching: uncached  optimal: $optimal  timing: $t ==========" >> $OUTDIR/explain.log 2>&1
...
EOF
										...
										echo $dev $build $rows $dataset $relpages $workers $wm $eic $ioc $matches $distinct $run cached-os $optimal $t >> $OUTDIR/results.csv

										echo "========== dev: $dev  build: $build  rows: $rows  dataset: $dataset  relpages: $relpages  workers: $workers  work_mem: $wm  effective_io_concurrency: $eic  io_combine_limit: $ioc  matches: $matches  distinct: $distinct  run: $run  caching: cached-os  optimal: $optimal  timing: $t ==========" >> $OUTDIR/explain.log 2>&1
										cat timing.log >> $OUTDIR/explain.log 2>&1
...
EOF
										...
										echo $dev $build $rows $dataset $relpages $workers $wm $eic $ioc $matches $distinct $run cached $optimal $t >> $OUTDIR/results.csv

										echo "========== dev: $dev  build: $build  rows: $rows  dataset: $dataset  relpages: $relpages  workers: $workers  work_mem: $wm  effective_io_concurrency: $eic  io_combine_limit: $ioc  matches: $matches  distinct: $distinct  run: $run  caching: cached  optimal: $optimal  timing: $t ==========" >> $OUTDIR/explain.log 2>&1
										cat timing.log >> $OUTDIR/explain.log 2>&1
