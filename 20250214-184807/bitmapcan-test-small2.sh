			...
			for dataset in uniform linear linear-fuzz cyclic cyclic-fuzz; do
				...
				for workers in 0 4; do
					...
					for wm in $((4*1024)); do
						...
						for eic in 0 1 16; do

							for ioc in 16 1 32; do

								for ra in 256 4096 16; do

									sudo blockdev --setra $ra /dev/md126

									for build in master patched-no-sequential; do
...
EOF
											...
											echo "========== dev: $dev  build: $build  rows: $rows  dataset: $dataset  relpages: $relpages  workers: $workers  work_mem: $wm  effective_io_concurrency: $eic  io_combine_limit: $ioc  ra: $ra  matches: $matches  distinct: $distinct  run: $run  caching: uncached  optimal: $optimal  timing: $t ==========" >> $OUTDIR/explain.log 2>&1
